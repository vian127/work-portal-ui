		...
		<basic-container>
			...
			>
                ...
                <template slot="stationForm">
                  <station :stationIsEdit="stationIsEdit" :companyId="companyId" :stationData="stationData" @getStation="getStation"></station>
                </template>
                <!-- 干系人 -->
                <template slot="takeholderForm" v-if="isShow">
                  <stakeholder :isShowView="isShowView" :stakeholderData="stakeholderData" :stationData="stationData" :companyId="companyId" @getStakeholder="getStakeholder"></stakeholder>
                </template>
                <!-- 公司关系 -->
                <!-- 暂时先隐藏 -->
    ...
    export default {
        ...
        data() {
            return {
                ...
                isShow: false, // 是否展示干系人
                isShowView: false, // 是否是查看
                stationIsEdit: false, // 服务驻点是否可编辑
            }
        },
                ...
                this.handleIsShow(this.isShow);
                 getObj(row.id).then(function(res){
                    if(type === 'view'){
                        ...
                        that.stationIsEdit = true,
                        that.$refs.crud.rowView(row,index);
                    } else if(type == 'edit'){
                        that.isShowView = false;
                        that.stationIsEdit = true,
                        that.stakeholders = res.data.data.companyStakeholders || [];
                        that.station = res.data.data.serviceStations || [];
                        that.$refs.crud.rowEdit(row,index);
                    }
                    // 干系人
                   that.stakeholderData = res.data.data.companyStakeholders;
                   that.stationData = res.data.data.serviceStations;
                })
            },
            /**
             * 控制干系人和服务驻点显示和隐藏

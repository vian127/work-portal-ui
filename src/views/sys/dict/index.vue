        ...
        <el-col :span="12" v-if="this.clickObj">
          <el-card class="box-card">
            <div slot="header" class="clearfix">
              <h3><i class="el-icon-document-copy"></i> 键值列表，所属类型：{{this.clickObj.code}} </h3>
            ...
            <avue-crud ref="crud2"
				...
				<template slot-scope="scope" slot="rstate">
					<el-switch
							...
							active-color="#13ce66"
							inactive-color="#ff4949"
							@change="changeEnable2(scope.row)">
					</el-switch>
				</template>
            </avue-crud>
...
<script>
  import {getPage, addObj, putObj, delObj} from '@/api/sys/dict'
  import {getPage2, addObj2, putObj2, delObj2} from '@/api/sys/dictvalue'
  import {tableOption} from './dict'
  import {tableOption2} from './dictvalue'
  ...
  export default {
    ...
    computed: {
      ...
      permissionList() {
        return {
			addBtn: this.vaildData(this.permissions.parm_dict_add, false),
			delBtn: this.vaildData(this.permissions.parm_dict_del, false),
			editBtn: this.vaildData(this.permissions.parm_dict_edit, false),
			viewBtn: this.vaildData(this.permissions.parm_dict_get, false)
       };
      }
    },
    ...
    handleDel: function (row, index) {
      ...
      beforeOpen2(done) {
        ...
        this.$set(this.form2,"dictCode",this.clickObj.code)
        done()
      },

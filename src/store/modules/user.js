...
//import {GetMenu} from '@/api/admin/menu' //marked by stoneshi 20200131
    ...
    GetUserInfo({commit}) {
      return new Promise((resolve, reject) => {
        getUserInfo().then((res) => {
          ...
          commit('SET_TENANT_ID', data.loginTid)
          commit('SET_TENANT_NAME', data.tenantName)
          commit('SET_USER_INFO', data.userAccount)
          commit('SET_ROLES', data.roles || [])
          commit('SET_PERMISSIONS', data.permissions || [])
          console.debug("code = " + data.userAccount.code)
          resolve(data)
        }).catch(() => {
          reject()
        })
